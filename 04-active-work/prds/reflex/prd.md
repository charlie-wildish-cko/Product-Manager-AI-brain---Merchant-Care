...
| **Analytics & Reporting** | FR-4, FR-4a, FR-5, FR-6, FR-7, NFR-3 | Insights Query Interface, dashboards, weekly digest, VoC view    |
...
| FR-4           | P0           | Cross-pillar contact driver view (Care PM): the interface provides a cross-pillar view of all contact drivers ranked by volume and estimated cost, filterable by product team, product pillar, merchant segment, territory, and time period; month-on-month comparison is available for any selected filter combination | Care PM can produce a ranked top-10 driver list filtered to any combination of pillar, segment, and territory without BQ access; MoM delta is visible on each driver; view is shareable via link | Analytics & Reporting |
| FR-4a          | P0           | Product team contact driver view (Product Manager): each Product team has a scoped view showing only the contact drivers attributed to their products, ranked by volume and estimated cost ($40/contact basis), with month-on-month trend; filters available: product, merchant segment, territory, time period; a PM can access their view without Care PM involvement | At least one PM outside the Care team can navigate to their product team's view, apply filters, and read their top drivers with cost and MoM trend without support from Charlie or the Data Scientist | Analytics & Reporting |
| FR-5           | P1           | Shareable driver snapshots: any filtered view in the interface generates a shareable link that preserves the filter state, so Care PM can send a PM a pre-scoped view ahead of a quarterly review without the PM needing to configure it themselves | A link generated by Care PM opens the correct filtered view for a PM who has not previously used the interface | Analytics & Reporting |
...
- **Cross-pillar view (Care PM):** All contact drivers ranked by volume and estimated cost across all product teams. Filterable by product team, pillar, merchant segment, territory, and time period. MoM delta visible on each driver.
- **Product team view (PM):** Scoped to a single product team's attributed drivers. Same filters and MoM trend. Accessible directly by the PM without Care PM involvement.

**Filter dimensions (both views):** Product team · Product pillar · Merchant segment · Territory · Time period

**Comparison:** Month-on-month toggle available on all views. Fixed periods (current month, last month, QTD) sufficient for Phase 2 — custom date ranges TBC.

**Shareability:** Any filtered view generates a persistent link preserving filter state. Primary use case: Care PM sends PM a pre-scoped link before a quarterly review.

**Access:** Role-based — PMs see their product team's view by default; Care PM has cross-pillar access. No BQ access required.
