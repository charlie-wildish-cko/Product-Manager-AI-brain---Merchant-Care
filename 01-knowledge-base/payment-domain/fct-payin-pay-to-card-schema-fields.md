...
> dbt docs blocks (`{% docs %}`) defining every column on `fct_payin` (card and APM payins) and `fct_pay_to_card` (Pay to Card payouts only — does NOT cover `fct_pay_to_bank`/bank payouts, no such fields are defined in this file). Use this as the authoritative field-level reference when defining or explaining a payin or Pay to Card field — do not paraphrase a field definition from memory if it exists here.
>
> Source: [cko-bi/gcp-data-analytics-dbt](https://github.com/cko-bi/gcp-data-analytics-dbt/blob/main/docs/payment_docs.md) (`docs/payment_docs.md`, `main` branch). No `fct_pay_to_bank` docs file exists anywhere in that repo's `docs/` directory as of 2026-09-07 — confirmed via `gh api repos/cko-bi/gcp-data-analytics-dbt/contents/docs`. Other doc files in that same directory (not yet pulled in here): `dispute_docs.md`, `reported_fraud_docs.md`, `risk_assessment_docs.md`, `scheme_monitoring.md`, `transaction_summary_docs.md`, `metadata_docs.md`, `source_override_mechanism.md`, `cko_native_implementation.md`, `data_products/data_products.md`, `data_products/data_product_standards.md`.
>
> To re-pull: `unset GITHUB_TOKEN` (a stale env token overrides the working `gh` keyring auth), then `gh api repos/cko-bi/gcp-data-analytics-dbt/contents/docs/payment_docs.md --jq '.content' | base64 -d`.
