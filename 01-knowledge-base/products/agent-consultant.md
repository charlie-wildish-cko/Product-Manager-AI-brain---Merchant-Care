...
The Consultant also functions as the backend for **Customer Agent** — the Care-owned data and reasoning layer that Fin (and future AI agents) call for any merchant data query. Rather than Fin calling other teams' APIs directly, Fin calls Customer Agent endpoints for payment status, settlement, balance, and dispute queries. Customer Agent retrieves data from BigQuery and internal systems, reasons over it, and returns a structured explanation. Fin converts that explanation into merchant-facing language. Sensitive data never leaves Care's systems; Fin receives only the output. This architecture removes cross-team API dependencies, uses BigQuery access the Consultant already holds, and is portable — any future AI agent can replace Fin without changing the Customer Agent interface.
