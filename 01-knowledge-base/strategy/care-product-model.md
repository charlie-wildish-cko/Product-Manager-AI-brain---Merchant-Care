...
Fin does not retrieve or reason over data directly. For any data-dependent query, Fin calls Care-owned Consultant endpoints. The Consultant retrieves from BigQuery and internal systems, reasons over the results, and returns a structured explanation. Fin converts that explanation into merchant-facing language. Raw data never leaves Care's systems. This removes cross-team API dependencies, uses BigQuery access the Consultant already holds, and is portable — any future AI agent can replace Fin without changing the Consultant interface. See [Agent Consultant](../products/agent-consultant.md).
