...
Fin does not retrieve or reason over data directly. For any data-dependent query, Fin calls **Customer Agent** — a Care-owned data and reasoning layer. Customer Agent retrieves from BigQuery and internal systems, reasons over the results, and returns a structured explanation. Fin converts that explanation into merchant-facing language. Raw data never leaves Care's systems. This removes cross-team API dependencies, uses BigQuery access the Consultant already holds, and is portable — any future AI agent can replace Fin without changing the Customer Agent interface. See [Agent Consultant](../products/agent-consultant.md).
