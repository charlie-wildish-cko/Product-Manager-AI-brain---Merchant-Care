...
**Last updated**: [DATE]
**For**: [NAME] — new engineer joining the Care Product team
...
**Team composition ([YEAR])**:
- 1 PM: Charlie Wildish
- [N] engineers + 1 Engineering Manager
...
We are in [YEAR]. The model has strong foundations in Agent Experience and Orchestration, but significant gaps in Fuel (data latency is a known blocker) and Insight & Prevention (Reflex is in delivery). The 2030 target state is an AI-first support operation — Fin resolves 80%+ of contacts autonomously, Agent Consultant handles 90%+ of routine agent tasks, and Reflex operates as an autonomous insights agent generating weekly action plans and fix PRs for product teams.

| Capability | [YEAR] (now) | 2030 (target) |
|---|---|---|
| AI resolution | Fin on Dashboard + email, partial coverage. Resolves a minority of contacts | Fin is the majority channel across B2B, B2C, and Platforms. 80%+ autonomous resolution |
| Agent data tooling | Merchant 360 context in sidebar. Payment data accessible but with latency | Real-time data across all sources, queryable via natural language. Proactive anomaly alerts before agent reads the ticket |
...
## [YEAR] Roadmap: What We're Building

<!-- Update this section each year / each half. Copy deliverable names verbatim from 2026 deliverables.md. -->

### Goal 1: Reduce / Maintain Contact Rate

| Deliverable | Status | What it is |
|---|---|---|
| [Deliverable name] | [Quarter] | [One-line description] |
